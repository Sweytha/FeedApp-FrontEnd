...
const AddFeed = () => {
    ...
    //closeModal is a function that is used to close the modal window and reset the image related state variables to their default values:
    const closeModal = () => {
    ...
    const onSelectFile = async (e) => {
        if (e.target.files && e.target.files.length > 0) {
            ...
            //opens the image cropper modal by setting the modalIsOpen state to true.
            setIsOpen(true);
        }
    };
