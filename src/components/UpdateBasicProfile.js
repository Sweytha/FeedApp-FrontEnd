...
//api to update basic profile from ApiUtil.js
...
const UpdateBasicProfile = ({
    ...
    //Formik component instance.
    const formikRef = useRef();

    // is a state variable that indicates whether an API request is in progress.
    const [isFetching, setIsFetching] = useState(false);

    //the child component is using AppContext object by using useContext hook
    const appContext = useContext(AppContext);
    //to retreive the token the cpokies
    ...
    //set the validation for the fields using Yup API
